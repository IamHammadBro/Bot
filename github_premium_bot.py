...
async def main():
    ...
    # Check if config is passed as command line argument
    if len(sys.argv) > 1:
        try:
        ...
        # Default config (your original credentials)
        config = {
    ...
    print(f"🎵 Starting bot with playlist: {config.get('playlist_url', 'Unknown')}")
    print(f"⚙️ Skip probability: {config.get('skip_probability', 0.55)*100}%")
